...
public class MagneticGripper : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Input.GetKey(attractKey))
		{
			if (grabbedRb == null)
			{
				// Aún no hay objeto agarrado: atrae; si entra en snapDistance, bloquea.
				AttractNearbyAndMaybeGrab();
			}
			else
			{
				// Ya hay objeto agarrado: no aplicamos fuerzas; se queda bloqueado por parent
	...
	{
		if (!grabbedRb) return;
		// Al estar parented con worldPositionStays=false ya se mantiene solo.
		// Si quieres redundancia estricta:
		grabbedRb.transform.localPosition = Vector3.zero;
		grabbedRb.transform.localRotation = Quaternion.identity;
		// Sin fuerzas ni nada: está bloqueado.
	}
	...
	void AttractNearbyAndMaybeGrab()
	{
		...
		// Si ya está en distancia de snap -> bloquear inmediatamente
		...
		// Si tras empujar este frame entra en snap, bloquea
